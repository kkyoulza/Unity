...
또한, 플레이어에게 RigidBody를 설정 해 준 다음, Mass를 10으로 설정 해 준다.
...
이제 적용 할 발판에 Physic Material을 드래그 앤 드랍 해 준다.
...
이렇게 코드를 적어 준 다음, Managing은 빈 오브젝트를 만들어 넣어 준다.

그 다음, 맨 아래 깔려 있던 빨간 판 tag를 under로 적용시켜 주면 초록 발판에서 떨어지게 되면 다시 출발 지점으로 오게 됨을 볼 수 있다.

이제, 발판들을 골인 지점까지 만든 다음, 세이브 포인트도 만들어 보도록 하자.
