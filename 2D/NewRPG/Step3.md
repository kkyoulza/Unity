...
따라서 오브젝트 풀링을 사용 해 주면 아래 그림과 같이 소환했던 데미지 텍스트를 재 사용하여 최소한으로 데미지 텍스트를 소환 해 주게 된다.
...
2번에서는 1번 배열 안에 있는 Prefab의 수 만큼 즉, 1번 배열의 크기만큼 리스트의 배열을 만들어 준다.

그 다음, 리스트에서는 사용할 수 있는 오브젝트가 있는지 확인 한다. (비활성화 되어 있는 상태의 놀고 있는 오브젝트)
